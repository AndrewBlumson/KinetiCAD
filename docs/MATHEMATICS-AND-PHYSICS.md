...
This is the equation and numerical-evidence reference for KinetiCAD. Sections 1–12 were reviewed against commit `8e954ab89cbc7ab0753b4d3145622c76a0fd63ef` on 12 September 2026; section 13 records the later bounded four-bar addition against its own source snapshots. KinetiCAD was originally built with **Replit Agent**; the later development and verification described here were carried out with **Codex**. Those development tools are not the physical model: the application uses OpenCascade for B-rep geometry and Rapier for rigid-body dynamics.

**Passing the stated tests establishes the stated cases and tolerances, not universal mathematical or physical certification.** This document was assembled by reading source, tests, retained logs and JSON reports. The historical standalone scenario reports were not rerun for the original documentation audit; its aggregate test suite was rerun separately and is recorded in the [earlier test catalog](TEST-CATALOG.md) and [machine-readable inventory](test-inventory-results.json). That earlier run passed **298 tests across 39 files in 146.122 seconds**. The later four-bar stage passed **348 tests across 51 files**, recorded in its [complete catalog](FOUR-BAR-TEST-CATALOG.md) and [validation record](four-bar-validation.json). The latest Undo/Redo and selection regression inventory is in the [current catalog](HISTORY-SELECTION-TEST-CATALOG.md); its [editing verification](HISTORY-AND-SELECTION.md) links browser and build evidence. The [physics verification overview](PHYSICS-VERIFICATION.md) tracks stage regression/build status. Historical reports retain their own dates, source hashes and counts.
...
| Four-bar path designer | Bounded local geometric search, native four-body mechanism and actual solver tracer-position comparison; see section 13 | Exact reproduction of arbitrary drawings, proven global optimisation, drawing-speed matching and finite drive/load ratings |
...
## 13. Later addition: bounded four-bar path design

The [four-bar verification guide](FOUR-BAR-PATH-VERIFICATION.md) contains the full equations, source references, parameter limits, test enumeration, practical use guide and rerun commands. Its evidence is separate from the 298-test audit and historical hash comparison above. The later [complete regression catalog](FOUR-BAR-TEST-CATALOG.md) records **348/348 passing tests across 51 files**, with no failures, cancellations or skips; the [validation record](four-bar-validation.json) identifies the measured source snapshot. Current build and Chrome status belong in [current acceptance status](CURRENT-STATUS.md); a search or solver report alone does not establish those checks.

The designer fits one closed, non-self-intersecting planar target with a crank-rocker linkage. It constructs four editable native CAD bodies with four revolute joints, then drives only the input crank. The traced material point moves with the actual coupler body; no slider, rocker or tracer position is prescribed from the reference equation. The generated profile uses one six-second turn at 10 RPM, zero gravity and 120 Hz. Reverse 10 RPM is also a numerical acceptance case. Its specialised solver profile is 32 outer iterations, 16 internal PGS iterations and motor-velocity gain 1,000,000; ordinary assemblies retain their existing settings.

For fixed pivots O=(0,0), D=(d,0), input pin A=a(cos θ,sin θ), coupler length b and rocker length c, write r=|D−A|, e=(D−A)/r and J for a 90° rotation. The two circle-intersection assemblies and coupler marker are:

$$
x=\frac{b^2-c^2+r^2}{2r},\qquad h=\sqrt{b^2-x^2},\qquad
B=A+xe+\sigma hJe,\quad \sigma\in\{-1,+1\},
$$
$$
P=A+\frac{u}{b}(B-A)+\frac{v}{b}J(B-A).
$$

The chosen branch stays fixed. Strict triangle inequalities over the entire crank turn, a uniquely shortest input crank, a positive Grashof margin and a transmission angle between 20° and 160° exclude the accepted family's change points. The 2 mm geometric margins and 20° threshold are explicit product choices, not certification standards. Separate conservative layer/capsule bounds protect the authored supports, links and pins; exact OCCT sampled intersections are additional evidence, not a continuous collision simulation. [Kinematics source](../artifacts/kineticad/src/mechanisms/fourBarKinematics.ts), [native factory](../artifacts/kineticad/src/mechanisms/fourBarAssembly.ts)

The deterministic worker search uses 64 candidates and 128 differential-evolution generations, for 8,256 evaluations. It scores the complete loop with cyclic starting-point alignment in both traversal directions; the reported dense result uses 256 uniform arc-length samples. For the aligned samples, RMS is `sqrt(mean(|Pᵢ−Tᵢ|²))` and worst sampled gap is `max(|Pᵢ−Tᵢ|)`. A separately retained symmetric point-to-segment gap includes both curves. These are geometric, sampled errors; neither drawing speed, continuous maximum error nor global optimality is established. The three known-mechanism seeds are disclosed. [Search source](../artifacts/kineticad/src/mechanisms/fourBarSynthesis.ts), [actual search worker](../artifacts/kineticad/src/mechanisms/fourBarSearchWorker.ts)

The [search report](four-bar-search-results.json) records 13 passing mathematics/search/worker tests, with seeds, source hashes and complete TAP output. The seed-42 ellipse search measured RMS **0.0882973 mm** and worst sampled gap **0.1770502 mm**, against 0.3 mm and 0.6 mm gates. A separate held-out mechanism path supplied only as points, absent from the known seed set, measured **0.0574099 mm** RMS and **0.1269418 mm** worst gap, against 1 mm and 3 mm gates. Those are two selected fits, not attainable-error promises for every drawing. Independent derivative tests check the analytical marker velocity and acceleration; the product solver comparison claims marker **position**, not instantaneous acceleration accuracy.

| Additional evidence | Measured scope and acceptance |
| --- | --- |
| [Actual native geometry](four-bar-geometry-results.json) / [test](../artifacts/kineticad/tests/four-bar-cad.test.mjs) | Seven assemblies, 28 valid single solids, 168 sampled poses and 1,008 exact pair intersections; maximum overlap 0 mm³ against a 10⁻⁵ mm³ gate. Seven exact marker-material probes confirm that the traced point belongs to the coupler. |
| [Actual rigid-body motion](four-bar-physics-results.json) / [test](../artifacts/kineticad/tests/four-bar-physics.test.mjs) | 34 scenarios across both branches, ±10 RPM, selected geometry bounds, timestep refinement and message partitioning. Maximum measured-angle marker error 0.000188749 mm / 0.05 mm gate; nominal-time marker error 0.0772475 mm / 0.1 mm; joint closure 0.000108791 mm / 0.05 mm; motor error 0.00402448 rad/s / 0.005 rad/s. Position/closure checks include startup; motor checks start after 0.25 s. |
| [Shipped CAD worker and preflight](four-bar-worker-cad-results.json) / [test](../artifacts/kineticad/tests/four-bar-worker-cad.test.mjs) | Eight single solids from two assemblies; six independently integrated bed/crank/rocker volumes differ by at most 3.092282×10⁻¹¹ mm³ against 10⁻⁷ mm³. Same-solid mass, COM, principal inertia/frame comparisons and disconnected-candidate rejection also pass. The coupler reference is a separate exact-CAD rebuild, not an analytical integral. |

Four near-boundary cases reduced nominal-position error by more than 20% when refined from 120 to 240 Hz. The default irregular-message run has bit-identical final transforms to the fixed-step run. These checks address selected numerical convergence and timing cases; they do not establish every possible edited geometry. The physics test may reuse exact-CAD descriptors only when its declared source and local geometry/material hashes match; cache reuse is not a new kernel rebuild.

The linked reports retain their measurement timestamps and full source hashes. Their combined CAD hash, specialised solver profile and worker snapshots are identified in the [four-bar provenance section](FOUR-BAR-PATH-VERIFICATION.md#reproduction-and-provenance). They do not update or supersede older report snapshots in section 12. Manual changes to the generated geometry or physical configuration disable its specialised reference/profile until the canonical configuration is restored.

This addition remains an ideal rigid mechanism. It does not add ordinary CAD contact, bearing friction, finite motor torque, payload capacity, elasticity or strength analysis. The independent actuator, contact and beam experiments retain their separate scopes.


## 14. Document restoration and object selection

These changes preserve source geometry and improve editing; they add no new physical model. [Document-history tests](../artifacts/kineticad/tests/document-history.test.mjs) and [store-history tests](../artifacts/kineticad/tests/history-store.test.mjs) check exact serialized source restoration, bounded history, branching, complete imported assets, saved simulation settings and rejection of stale asynchronous restores. Runtime frames and derived mass/mesh caches are excluded; restored source is rebuilt and the simulation returns to its stopped design pose.

[Selection tests](../artifacts/kineticad/tests/object-selection.test.mjs) use Three.js meshes, camera matrices and ray intersections. For an input ray `r(t) = o + t d`, selection is the nearest positive visible triangle intersection among displayed native and Boolean results. A bounding box alone is not a hit: holes remain empty unless another visible solid is behind them. The selected outline follows actual CAD edge coordinates and the displayed object's world matrix; it is not a collider. Native part transforms remain editable; derived Boolean output is edited through its inputs.

History restoration compares exact document values. That differs from floating-point geometry/dynamics comparisons, which retain the tolerances documented above. See [current test names and results](HISTORY-SELECTION-TEST-CATALOG.md) and [actual Chrome observations](HISTORY-AND-SELECTION.md). Historical physics report values and source fingerprints have not been relabelled as new measurements.
