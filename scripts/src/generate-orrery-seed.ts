...
const PARAMS = {
  ...
  moonOnPlanets: [2, 4, 6] as number[],     // Phase 1: [] | Phase 3: [2,4,6] (1-based)
  ring: true,                               // Phase 1: false | Phase 4: true
  ...
  // Moon geometry is lifted moonLiftMm above the moon body's local origin so it
  // sweeps above the planet disc (planetHeight mm tall) rather than through it.
  // moonLiftMm = planetHeight keeps the moon flush with the planet disc top.
  moonLiftMm: 8,                            // mm local-Z lift of moon arm + disc geometry
  ...
  ringLumpRadius: 3,                        // mm base radius per lump disc (varied ±1mm)
  ringLumpHeight: 4,                        // mm height per lump disc
  // Ring sits at Z=7mm: below tier 1 (Z=15mm) so it sweeps under all eight arms.
  // Same pattern as moonLiftMm: body positionMm Z and sun-side pivot Z both = ringZ.
  ringZ: 7,                                 // mm world Z of the ring body origin
...
function buildMoonPart(moonIndex: number, planetIndex: number): object {
  ...
  // The moon body's local origin must coincide with the planet disc centre in world
  // space so that the two joint pivots (pivotA=[r,0,moonLiftMm] in planet local,
  // pivotB=[0,0,0] in moon local) resolve to the same world point.
  // Planet disc centre world XY = fan-angle rotation of [orbitRadius, 0, 0].
  // World Z = tierHeight(planetIndex) + moonLiftMm so the moon geometry (at local
  // Z=0) sweeps above the planet disc top rather than through it.
  const r = orbitRadius(planetIndex);
  const fanAngle = ((planetIndex - 1) * 45 * Math.PI) / 180;
  const moonWorldX = r4(r * Math.cos(fanAngle));
  const moonWorldY = r4(r * Math.sin(fanAngle));
  const moonWorldZ = tierHeight(planetIndex) + PARAMS.moonLiftMm;
...
function buildRingPart(): object {
  ...
  for (let k = 0; k < PARAMS.ringLumpCount; k++) {
    ...
    // Per-lump radius variation using the golden-ratio angle as a period to avoid
    // obvious repetition. Ranges approximately ±1mm around ringLumpRadius.
    const lumpR = r4(PARAMS.ringLumpRadius + Math.sin(k * 1.618) * 1);

    sketches.push({
      id: `sk-ring-lump-${k}`,
      name: `Ring lump ${k + 1}`,
      plane: "XY",
      primitives: [{ type: "circle", centre: [cx, cy], radius: lumpR }],
    });
...
function buildMoonMate(moonIndex: number, planetIndex: number): object {
  // Pivot on the parent arm-planet part: [orbitRadius, 0, moonLiftMm] in planet
  // local frame — the planet disc centre XY plus the lift offset so the pivot
  // coincides with the moon body's local origin in world space.
  // Pivot on moon body: [0,0,0] (moon local origin, per spec section 6.5).
  const r = orbitRadius(planetIndex);
  return {
    ...
    pivotA: {
      ...
      localPoint: [r, 0, PARAMS.moonLiftMm],
    },
...
function buildRingMate(): object {
  return {
    ...
    partA: "part-sun",
    partB: "part-ring",
    // Sun-side pivot Z = ringZ so the pivot coincides with the ring body's world origin.
    // Ring-side pivot stays at ring-local [0,0,0].
    pivotA: { kind: "edge", edgeId: "sun-center", localPoint: [0, 0, PARAMS.ringZ] },
...
// Phase 4 settings: planetCount=${PARAMS.planetCount}, moonOnPlanets=[${PARAMS.moonOnPlanets.join(",")}], ring=${PARAMS.ring}.
