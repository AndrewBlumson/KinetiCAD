...
| Stewart platform | 14 / 18 | Six telescopic actuators drive a coordinated six-second vertical lift through a closed joint network |
...
gravity. The Stewart example commands equal actuator extension at 2 mm/s;
it does not provide arbitrary six-axis inverse kinematics or a payload rating.

The six-demo CAD and physics reports pass, including the material-force
experiment and Stewart's measured closed-joint lift. Exact CAD pair checks
find no overlap at either experiment's initial and measured final poses.
The rebuilt local Chrome route displays all six cards and the measured
Stewart lift. Public deployment acceptance remains separate; see
[Physics verification](docs/PHYSICS-VERIFICATION.md) for current results and
the retained earlier five-demo browser evidence.
...
there is no collision, friction, gear-tooth contact or bearing resistance.
There is no general-purpose clearance checker; specific demo geometry has
separate offline B-rep intersection and travel-envelope tests. Motors are velocity servos without enforced torque or
...
complete project recovery, and eventually a local four-bar linkage optimiser
that fits a user-drawn path and produces editable CAD. No AI API or
path-to-mechanism optimiser is included in this demo extension.
...
`docs/stewart-physics-results.json` records the actual closed-joint lift;
`docs/stewart-clearance-results.json` records every B-rep pair at its initial
and measured final poses. These endpoint checks do not establish continuous
collision detection or contact forces.
Run heavyweight OpenCascade commands sequentially.

The current unit-test inventory is 79 cases across nine suites, including
seven actual-OCCT arc/profile regressions. All 79 passed in serialized runs;
the mass and regeneration suites were rerun after the arc fix. These results
do not replace rendered or public deployment acceptance.
